...
class CIRCUIT
{
    ...
    public:
        ...
        void build_defied_queue_noPO(){
            Queue_defined =  new ListofGate[MaxLevel + 1];
            unsigned i = 0;
            GATE* gptr;
            ...
            for (; Circuit_ite != Netlist.end();Circuit_ite++, i++) {
                ...
                if(gptr->GetFunction()!=G_PO)
                    Queue_defined[gptr->GetLevel()].push_back(gptr);
            }
        ...
        //defined in atpg.cc
        void BridgingFault(fstream &outfile);
        void BridgingFault_e();
        void ParellelBridgingFault();
        void GenerateAllFaultList();
        void GenerateCheckPointFaultList();
        void GenerateFaultList();
        void Generate_bridge_fault_list();
        void Atpg();
        ...
        void BF_InjectFaultValue(GATEPTR gptr, unsigned idx,Bridging value);
