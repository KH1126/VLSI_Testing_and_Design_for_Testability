...
void CIRCUIT::BFaultSimVectors()
{
    ...
    if(!Pattern.eof()){ // Readin the first vector
        while(!Pattern.eof()){
            ...
            BFaultSim();
            int b=UBFlist.size();
            cout << "Found fault number :"<< a-b << endl;
...
{
    register unsigned i, fault_idx(0);
    GATEPTR gptr,gptr2,gptr_false;
    BridgingFAULT *fptr;
    BridgingFAULT *simulate_flist[PatternNum];
    list<GATEPTR>::iterator gite;
    //initial all gates
    //cout<<"test"<<endl;
    for (i = 0; i < Netlist.size(); ++i) {
    ...
    for (fite = UBFlist.begin();fite!=UBFlist.end();++fite) {
        ...
        //not relate to the branch or stem -> all fault seen as stem fault start ========================
        //firstly find which one to be set as fault
        
        if(static_cast<int>(fptr->GetType()) != static_cast<int>(gptr->GetValue()))
             gptr_false=gptr;
        else
             gptr_false=gptr2;
       
        if (!gptr_false->GetFlag(FAULTY)) {
            //cout<<"test"<<endl;
            gptr_false->SetFlag(FAULTY); GateStack.push_front(gptr_false);
        }
        BF_InjectFaultValue(gptr_false, fault_idx, fptr-> GetType());
        gptr_false->SetFlag(FAULT_INJECTED);
        ScheduleFanout(gptr_false);
        simulate_flist[fault_idx++] = fptr;
        //end ========================
