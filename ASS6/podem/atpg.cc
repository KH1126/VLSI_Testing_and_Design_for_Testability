...
void CIRCUIT::setFault(GATEPTR gptr, GATEPTR fanout, VALUE type) {
    FAULT *fptr = new FAULT(gptr, fanout, type);
    Flist.push_front(fptr);
    UFlist = Flist;
    return;
}
...
{
    cout << "Run stuck-at fault randomp pattern gerenration for 1000 times or 90% fault coverage" << endl;
    ...
    {
        for (i = 0;i < PIlist.size();++i){
            PIlist[i]->SetValue(VALUE(rand()%2));
        }
        ...
        unsigned detected_num(0), undetected_num(0);
        for (fite = Flist.begin();fite!=Flist.end();++fite) {
            fptr = *fite;
        ...
        if(detected_num/double(total_num) > 0.9) {
            cout << "---- fault coverage reach 90%"<<" and break ----";
            break;
        }
    }
    //compute fault coverage
...
void CIRCUIT::Atpg()
{
    ...
    ofstream OutputStrm;
    if (option.retrieve("output")){
        if(option.retrieve("random_pattern")) OutputStrm.open((char*)option.retrieve("output"), ios::app);
        else OutputStrm.open((char*)option.retrieve("output"), ios::out);
        if(!OutputStrm){
              cout << "Unable to open output file: "
                   << option.retrieve("output") << endl;
              cout << "Unsaved output!\n";
    ...
    //if(option.retrieve("hw6-c")) cout<<"---------------------"<<endl;
    cout.setf(ios::fixed);
    cout.precision(2);
    cout << "---------------------------------------" << endl;
...
ATPG_STATUS CIRCUIT::Podem(FAULT* fptr, unsigned &total_backtrack_num)
{
    ...
    //hw6-c
    bool traceflag=0;
    if(option.retrieve("hw6-c")) traceflag=1;
    ...
    switch (status) {
        case TRUE:
            ...
            if (FaultEvaluate(fptr)) {
                //forward implication
                //if(traceflag) { cout << "+ FaultEvaluate : true" << endl; }
    ...
    while(backtrack_num < BackTrackLimit && status == FALSE) {
        ...
        if (pi_gptr) { //decision found
            ...
            if(traceflag) cout<<"Backtrack: "<<endl;
        ...
        if (pi_gptr) {
            if(traceflag) cout<<"Forward Simulation: "<<endl;
            LogicSim();
            //fault injection
            if(FaultEvaluate(fptr)) {
                //forward implication
            ...
            if (CheckTest()) 
            { 
                if(traceflag) cout<<"Fault has propagated to output"<<endl;
                status = TRUE;
            }
        }
    } //end while loop
...
ATPG_STATUS CIRCUIT::SetUniqueImpliedValue(FAULT* fptr)
{
    ...
    if (!fptr->Is_Branch()) {
        //if(option.retrieve("hw6-c")) cout << "!fptr->Is_Branch()  SetUniqueImpliedValue " << status << endl;
        return status; 
    }
...
bool CIRCUIT::FaultEvaluate(FAULT* fptr)
{
    ...
    if (ivalue == X || ivalue == fptr->GetValue()) { return false; }
        ...
        if(option.retrieve("hw6-c")) cout<<"Fault Activiation: let "<<igptr->GetName()<<" to 1"<<endl;
        igptr->SetValue(D); 
    }
    else if (ivalue == S0) { 
        if(option.retrieve("hw6-c")) cout<<"Fault Activiation: let "<<igptr->GetName()<<" to 0"<<endl;
        igptr->SetValue(B); 
    }
...
GATEPTR CIRCUIT::TestPossible(FAULT* fptr)
{
    ...
    if (!ogptr->GetFlag(OUTPUT)) {
        if (ogptr->GetValue() != X) {
            ...
            if(option.retrieve("hw6-c")) cout<<"Fault Propagation "<<decision_gptr->GetName()<<endl;
    ...
    if(option.retrieve("hw6-c")) {
        cout << "LET " << decision_gptr->GetName() << " to be " << decision_value << endl;
        cout << "Backtrace " <<endl;
    }
...
GATEPTR CIRCUIT::FindPIAssignment(GATEPTR gptr, VALUE value)
{
    ...
    if(option.retrieve("hw6-c")) {
        cout << "FindPIAssignment process returns " << j_gptr->GetName() << " " <<  j_value << endl;
    }
...
void CIRCUIT::BridgingFault_e()
{
    ...
    for (unsigned i = 0; i <= MaxLevel; i++) {
        while (LevelQueue[i].size() >= 2) {
            ...
            LevelQueue[i].pop_front();
            n1 = LevelQueue[i].front();
            BFlist.push_front(new BridgingFAULT(n0, n1, AND));
            BFlist.push_front(new BridgingFAULT(n0, n1, OR));
        }
    }
    cout<<"BFLIST: "<<BFlist.size()<<endl;
    UBFlist = BFlist;
}
...
void CIRCUIT::Atpg_bridge()
{
    ...
    unsigned i, total_backtrack_num(0), pattern_num(0);
    ATPG_STATUS status_0,status_1;
    
    //ATPG_STATUS status;
    BridgingFAULT* fptr;
    list<BridgingFAULT*>::iterator fite;
    //Prepare the output files
    ofstream OutputStrm;
    if (option.retrieve("output")){
        OutputStrm.open((char*)option.retrieve("output"),ios::out);
    ...
    for (fite = BFlist.begin(); fite != BFlist.end();++fite) {
        ...
        FAULT* fptr2;
        if(fptr->GetType() == AND){
          fptr1 =new FAULT(fptr->Getn0(), fptr->Getn0(), S0);
          fptr2 =new FAULT(fptr->Getn1(), fptr->Getn1(), S0);
        
        }
        else if(fptr->GetType() == OR){
          
          fptr1 =new FAULT(fptr->Getn0(), fptr->Getn0(), S1);
          fptr2 =new FAULT(fptr->Getn1(), fptr->Getn1(), S1);
        
        }
        status_0 = Podem(fptr1,total_backtrack_num);
        status_1 = Podem(fptr2,total_backtrack_num);
        switch (status_0) {
            case TRUE:
                fptr->SetStatus(DETECTED);
                //run fault simulation for fault dropping
                if(fptr->Getn1()->GetValue() ==  fptr->Getn0()->GetValue()) break;
                for (i = 0;i < PIlist.size();++i) { 
			            ScheduleFanout(PIlist[i]);       
                  if (option.retrieve("output")){ OutputStrm << PIlist[i]->GetValue();}
		            }
        ...
        switch (status_1) {
            case TRUE:
                if(fptr->GetStatus() == DETECTED) break;
                if(fptr->Getn1()->GetValue() ==  fptr->Getn0()->GetValue()) break;
                fptr->SetStatus(DETECTED);
                //run fault simulation for fault dropping
                for (i = 0;i < PIlist.size();++i) { 
