...
int SetupOption(int argc, char ** argv)
{
    ...
    option.enroll("check_point", GetLongOpt::NoValue,
            "build check_point fault list", 0);
    option.enroll("bridging", GetLongOpt::NoValue,
            "build bridging fault list", 0);
    option.enroll("bridging_fsim", GetLongOpt::NoValue,
            "bridging faults simulation ", 0);
    option.enroll("hw6-c",GetLongOpt::NoValue,
            "hw6-c simulation", 0);
    option.enroll("random_pattern", GetLongOpt::NoValue,
            "ASS6-Problem_D: random pattern generation before ATPG", 0);
...
int main(int argc, char ** argv)
{
    ...
    {
        string filename = option.retrieve("output");
        fstream outfile;
        outfile.open(filename.c_str(), ios::out);
        Circuit.BridgingFault(outfile);
        outfile.close();
    }
    else if (option.retrieve("bridging_fsim")){
        Circuit.ParellelBridgingFault();
        Circuit.SortFaninByLevel();
        Circuit.MarkOutputGate();
        Circuit.InitPattern(option.retrieve("input"));
    ...
    {   
        Circuit.BridgingFault_e();
        Circuit.SortFaninByLevel();
        Circuit.MarkOutputGate();
        Circuit.Atpg_bridge();
    }
    else if (option.retrieve("hw6-c")){
        int number1=0,number2=0;
        for(int i=0; i<Circuit.No_Gate(); i++) {
            if(Circuit.Gate(i)->GetName()=="n60") number1=i;
            else if (Circuit.Gate(i)->GetName()=="net17") number2=i;
        }
        // net17 stuck-at-0
        cout << "s-a-0 fault at " << Circuit.Gate(number2)->GetName() << endl;
        Circuit.setFault(Circuit.Gate(number2), Circuit.Gate(number2), S0);

        // n60 stuck-at-1
        cout << "s-a-1 fault at " << Circuit.Gate(number1)->GetName() << endl;
        Circuit.setFault(Circuit.Gate(number1), Circuit.Gate(number1), S1);
